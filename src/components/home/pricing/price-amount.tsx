  ...
  return (
    <div className="mt-6 flex flex-col px-8">
      {loading ? (
        ...
        <>
          <div className={cn('text-[80px] leading-[96px] tracking-[-1.6px] font-medium text-black')}>
            {priceMap[tier.priceId[value]].replace(/\.00$/, '')}
          </div>
          <div className={cn('font-medium leading-[12px] text-[12px] text-black')}>{priceSuffix}</div>
        </>
      )}
    </div>
  );
